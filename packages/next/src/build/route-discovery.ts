...
export async function discoverRoutes(
  ...
  const { appDir, pagesDir, pageExtensions, isDev, baseDir, isSrcDir } = options

  const validFileMatcher =
    options.validFileMatcher || createValidFileMatcher(pageExtensions, appDir)

  const appDirOnly = !!appDir && !pagesDir
  ...
  if (pagesDir) {
    ...
    mappedPages = await mapPaths(pagePaths, PAGE_TYPES.PAGES)
  ...
  if (appDir) {
    ...
    ;[mappedAppPages, mappedAppLayouts] = await Promise.all([
      mapPaths(appPaths, PAGE_TYPES.APP),
      mapPaths(layoutPaths, PAGE_TYPES.APP),
    ])
    const mappedDefaultFiles = await mapPaths(defaultPaths, PAGE_TYPES.APP)
